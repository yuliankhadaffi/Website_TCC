...
$pjlomba = mysqli_real_escape_string($koneksi, isset($_POST['pjlomba']) ? $_POST['pjlomba'] : '');
$biaya = mysqli_real_escape_string($koneksi, isset($_POST['biaya']) ? $_POST['biaya'] : '');
...
if($_FILES['foto']['error'] == 0){
	if($_FILES['foto']['size'] < 1000000){
		...
		// ubah data
		$query = mysqli_query($koneksi, "UPDATE tbl_guru SET nama = '$nama', jenis_kelamin = '$jenis_kelamin', no_hp = '$no_hp', pjlomba = '$pjlomba', biaya = '$biaya', foto = '$nama_foto' WHERE id = $id") or die(mysqli_error($koneksi));
		if($query){
